...
const OrderManager = ({ restaurantId }) => {
  const { orders, loading, refetch, changeStatus } = useOrders(orderRepository, restaurantId);
        ...
        <div className="orders-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '1.5rem' }}>
          <AnimatePresence>
            {orders.map((order, idx) => {
              const statusMeta = ORDER_STATUS_META[order.status] || { name: order.status, color: 'var(--border)' };
              return (
                <motion.div
                  key={order.id}
                  layout
                  ...
                  style={{ padding: '1.5rem', borderLeft: `4px solid ${statusMeta.color}` }}
                >
